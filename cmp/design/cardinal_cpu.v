//CARDINAL CPU WITH FORWARDING AND LOAD STALL
module cardinal_cpu (
    clk, reset,
    inst_in, d_in, pc_out, addr_out, memEn, memWrEn, d_out,
    nicEn, nicWrEn, nic_addr, nic_d_out //FOR NIC
);
...
//FROM WB -> ID FORWARDING MUX
wire        [0:63] id_srcA_fwd;
wire        [0:63] id_srcB_fwd;

// FROM WB -> EX FORWARDING 
...
//FROM BRANCH DECISION
wire        branch_taken;

//FROM ALU INSTANTIATION
wire        [0:63] alu_results;

//FROM SFU INSTANTIATION
wire        [0:63] sfu_results;

//NIC ADDRESS DECODE WIRE
wire        is_nic_addr;
...
// Writeback value
// ============================================================
assign wb_data = EX_WB_memEn ? (EX_WB_is_nic ? nic_d_out : d_in) : EX_WB_result;
...
// NIC address decode
// ============================================================
assign is_nic_addr = ID_EX_imm_addr[0] & ID_EX_imm_addr[1];
...
assign memEn    = ID_EX_memEn & ~is_nic_addr;
assign memWrEn  = ID_EX_memWrEn;
assign addr_out = {16'b0, ID_EX_imm_addr};

// New NIC outputs:
assign nicEn    = ID_EX_memEn  & is_nic_addr;
assign nicWrEn  = ID_EX_memWrEn & is_nic_addr;
assign nic_addr = ID_EX_imm_addr[14:15];
// addr_out[30:31] = imm_addr[14:15] in big-endian
